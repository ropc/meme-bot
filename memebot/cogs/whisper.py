...
from typing import Optional, MutableSet, Dict, MutableMapping, Tuple, List, Any, Type
...
class Whisper(commands.Cog):
        ...
        with context.typing():
            ...
            for channel in category.text_channels:
                ...
                if len(possible_members) == 1:
                    ...
                    self.save_config()
                    await context.send(f'set personal channel {channel.mention} for {member.mention}')
                    continue
    ...
    @commands.command()
    @commands.has_role('whisper-manager')
    async def set_personal_channel(self, context: commands.Context, member: discord.Member, channel: Optional[discord.TextChannel],
            role: discord.Role, whisper_channel: Optional[discord.TextChannel], whisper_count: Optional[int] = 2):
        guild_config: GuildConfig = self.get_or_create_guild_config(
            guild_id=context.guild.id,
            whisper_channel_id=whisper_channel.id if whisper_channel else None,
        ...
        guild_config.member_configs[member.id] = WhisperConfig(
            member_id=member.id,
            personal_channel_id=channel.id,
            guild_id=context.guild.id,
            role_id=role.id,
    ...
    @commands.command()
    @commands.has_role('whisper-manager')
    async def set_whisper_count(self, context: commands.Context, count: Optional[int] = 2):
        guild_config = self.whisper_config.guild_configs.get(context.guild.id)
        if not guild_config:
        ...
        async def set_and_notify_whisper_count(member_config: MemberConfig):
            ...
            member_config.whisper_count = count
            ...
            return await personal_channel.send(f'you have {count} whispers')

        with context.typing():
            await asyncio.gather(*[set_and_notify_whisper_count(config) for config in guild_config.member_configs.values()])
            self.save_config()
